  ...
  return (
    <nav className={
      ...
      <Link to="/" className='flex items-center gap-2' onClick={()=>{setActive("");window.scrollTo(0,0);}}>
        ...
        <p className='text-white text-[18px] font-bold cursor-pointer flex'>Fredrick &nbsp;<span className='sm:block hidden'> | FullStack Developer</span></p>
      </Link>
      </div>
      <div>
      ...
      <div className={`${!toggle ? 'hidden':'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
        ...
        {navLinks.map((link)=>(
          <li key={link.id} className={`${active===link.title?"text-white":"text-secondary"} font-poppinsfont-medium cursor-pointer text-[16px]`} onClick={()=>{setToggle(!toggle);setActive(link.title);}}>  
            <a href={`#${link.id}`}>{link.title}</a>
          </li>
        ))}
      </ul>
